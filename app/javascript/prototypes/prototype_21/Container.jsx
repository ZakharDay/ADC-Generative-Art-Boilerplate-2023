...
export default class Container extends Component {
  ...
  handleChannelVolumeChange = (id, property, value) => {
    const channels = structuredClone(this.state.channels)

    channels.map((channel, i) => {
      if (channel.id === id) {
        console.log('yes 1')
        channel.settings[property] = value
      }
    ...
    const node = nodes[id]
    console.log(nodes, node, id, property)
  ...
  renderUI = () => {
    ...
    chains.forEach((chain, i) => {
      chainColumns.push(<ChainColumn chain={chain} key={i} />)
    })

    busses.forEach((bus, i) => {
      busColumns.push(<BusColumn bus={bus} key={i} />)
    })
    ...
    channels.forEach((channel, i) => {
      channelColumns.push(
        <ChannelColumn
          channel={channel}
          handleChannelVolumeChange={this.handleChannelVolumeChange}
          key={i}
        />
      )
    })

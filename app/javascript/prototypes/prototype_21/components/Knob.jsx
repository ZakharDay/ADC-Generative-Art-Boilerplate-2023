...
export default class SC_Knob extends PureComponent {
  ...
  calcValue = (deg) => {
    ...
    if (min < 0) {
      ...
      return (range / 280) * deg
    } else {
      // return Math.floor((range / 280) * (deg + 140))
      return (range / 280) * (deg + 140)
    }
  }
  ...
  calsBulletValues = () => {
    ...
    if (Math.sign(min) == 1) {
      ...
      if (Math.sign(max) == -1) {
        ...
        // bullets.push(Math.floor(stepCoef * 1))
        // bullets.push(Math.floor(stepCoef * 2))
        bullets.push(-stepCoef * 2)
        bullets.push(-stepCoef * 1)
        bullets.push(stepCoef * 0)
        bullets.push(stepCoef * 1)
        bullets.push(stepCoef * 2)
      }
  ...
  handleMouseMove = (e) => {
    const { id, property, min, max, handleChange } = this.props

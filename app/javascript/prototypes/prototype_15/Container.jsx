...
export default class Container extends Component {
  ...
  componentDidMount() {
    ...
    this.bufferSamples(this.state.drumsSettings.samples)
  }
  ...
  startBeat = () => {
    ...
    drumsPart = new Tone.Part((time, note) => {
    ...
    }, drumsSettings.sequences[0].sequence).start(0)

    drumsPart.loopEnd = drumsSettings.sequences[0].duration
    drumsPart.loop = drumsSettings.sequences[0].loop
  }
  ...
  handleDrumsSequenceChange = (property, value) => {
    const { drumsSettings } = this.state
    const { sequence, duration } = drumsSettings.sequences[value]
    const currentDrumSequence = value

    drumsPart.clear()
    drumsPart.loopEnd = duration

    console.log(value, drumsSettings.sequences[value], sequence)
  ...
  renderUI = () => {
    ...
    const drumsSequences = drumsSettings.sequences.map((s, i) => {
      return i
    })
        ...
        <div className="instrumentsWrapper">
          ...
          <div className="instrumentUI">
            ...
            <div className="controlsGroup">
              <SC_ToggleButtonSet
                name="Sequence"
                options={drumsSequences}
                value={currentDrumSequence}
                property="drumsSequence"
                handleChange={this.handleDrumsSequenceChange}
              />
            </div>
